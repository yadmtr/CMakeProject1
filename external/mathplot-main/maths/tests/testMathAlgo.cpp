...
int main()
{
    ...
    sm::range<float> mmvf = MathAlgo::maxmin (vf);
    ...
    sm::MathAlgo::boxfilter_2d<float, filter_width, data_width> (vals, filtered);
    ...
    sm::vvec<float> edata = { 1.0f, 0.5f, 1.0f,
                                 0.5f, 1.0f, 0.4f,
                                 1.0f, 0.5f, 0.9f };
    sm::vvec<float> hedges (edata.size(), 0);
    sm::vvec<float> vedges (edata.size(), 0);

    sm::vvec<float> vedges_exp = { -1.0f+0.5f, -0.5f+1.0f, -1.0f+1.0f,
                                      -0.5f+1.0f, -1.0f+0.4f, -0.4f+0.5f,
                                      -1.0f+0.5f, -0.5f+0.9f, -0.9f+1.0f };
    sm::vvec<float> hedges_exp = { -1.0+0.5f, -0.5f+1.0f, -1.0f+0.4f,
                                      -0.5f+1.0f, -1.0f+0.5f, -0.4f+0.9f,
                                      0,          0,          0}; // no wrapping in vertical axis so top row of hedges is 0


    // Default template
    sm::MathAlgo::edgeconv_2d<float, 3> (edata, vedges, hedges);
    ...
    sm::MathAlgo::edgeconv_2d<float, 3, invert_vert_edges> (edata, vedges, hedges);
    if (vedges != -vedges_exp) { --rtn; }
    if (hedges != hedges_exp) { --rtn; }

    // Test version which inverts horizontal edges
    constexpr bool invert_horz_edges = true;
    sm::MathAlgo::edgeconv_2d<float, 3, false, invert_horz_edges> (edata, vedges, hedges);
    if (vedges != vedges_exp) { --rtn; }
    if (hedges != -hedges_exp) { --rtn; }

    // Test version which inverts vertical and horizontal edges
    sm::MathAlgo::edgeconv_2d<float, 3, invert_vert_edges, invert_horz_edges> (edata, vedges, hedges);
    if (vedges != -vedges_exp) { --rtn; }
    if (hedges != -hedges_exp) { --rtn; }
