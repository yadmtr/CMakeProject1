...
namespace mplot
{
    ...
    //! Allocate storage and read in the data from the file at filepath.
    class AllocAndRead {
    public:
        // Construct an empty AllocAndRead object.
        AllocAndRead () : filepath (""), data_(nullptr) {}
        // Construct an AllocAndRead object and read the content of the file at @param path
        AllocAndRead (const std::string& path) : filepath (path), data_(nullptr) { this->read(); }
        ~AllocAndRead () { if (this->data_) { free (this->data_); } }
        ...
        /*!
         * Obtain an indexed character from @see data_.  @param i index into @see data_.  @return
         * the character at data_[i].
         */
        ...
        // Get a pointer to @see data_.  @return @see data_
        char* data() { return this->data_; }
        // Get the size of @see data_. This is stored in the member @see sz.  @return @see sz.
        std::size_t getsize() const { return this->sz; }
        // Reads the file at @param path into @see data_. Allocates memory as required.
                                              ...
                                              std::string::size_type& endOfCmd)
        {
            ...
            while (p1 != std::string::npos && numnum < numParams) {
                ...
                if (p1 != p0) {
                    ...
                    if (mplot::tools::containsOnlyWhitespace (ccs)) {
                        // Do nothing
                    } else {
                        ss >> n;
                        numbers.push_back (n);
        ...
        sm::bezcurvepath<float> parseD (const std::string& d)
        {
            ...
            while (p1 != std::string::npos) {
                ...
                case 'l': // lineto command, deltas
                {
                    ...
                    if (mplot::tools::containsOnlyWhitespace (lCmd)) {
                        p3 = lCmd.size()-1;
                    } else {
                ...
                case 'm': // move command, deltas
                {
                    ...
                    if (mplot::tools::containsOnlyWhitespace (mCmd)) {
                        p3 = mCmd.size()-1;
                    } else {
                ...
                case 'c': // cubic Bezier curve, deltas
                {
                    ...
                    if (mplot::tools::containsOnlyWhitespace (cCmd)) {
                        p3 = cCmd.size()-1;
                    } else {
                ...
                case 's': // shortcut cubic Bezier, deltas
                {
                    ...
                    if (mplot::tools::containsOnlyWhitespace (sCmd)) {
                        p3 = sCmd.size()-1;
                    } else {
